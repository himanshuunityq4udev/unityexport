...
public class AnimationHelper
{
    ...
    {
        RectTransform canvasRect = Transform.root.GetComponent<RectTransform>(); // Get the canvas' RectTransform
        CanvasScaler canvasScaler = Transform.root.GetComponent<CanvasScaler>(); // Get the CanvasScaler component
        float scaleFactor = canvasScaler.referenceResolution.x / canvasRect.rect.width; // Calculate scaling based on width
        ...
        switch (Direction)
        {
            ...
            case Direction.UP:
                startPosition = new Vector2(0, -canvasRect.rect.height);
                break;
            case Direction.RIGHT:
                startPosition = new Vector2(-canvasRect.rect.width, 0);
                break;
            case Direction.DOWN:
                startPosition = new Vector2(0, canvasRect.rect.height);
                break;
    ...
    public static IEnumerator SlideOut(RectTransform Transform, Direction Direction, float Speed, UnityEvent OnEnd)
    {
        ...
        {
            case Direction.UP:
                endPosition = new Vector2(0, canvasRect.rect.height);
                break;
            case Direction.RIGHT:
                endPosition = new Vector2(canvasRect.rect.width, 0);
                break;
            case Direction.DOWN:
                endPosition = new Vector2(0, -canvasRect.rect.height);
                break;
